  ...
  if (userError || !user) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="border border-accent-red p-4 text-accent-red text-sm font-mono">User not found.</div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col md:flex-row gap-8">
      <aside className="md:w-64 flex-shrink-0">
        ...
        <h1 className="text-xl font-semibold text-primary leading-tight">{user.display_name || user.username}</h1>
        <p className="font-mono text-secondary text-sm mt-0.5 mb-3">@{user.username}</p>
        {user.bio && <p className="text-sm text-secondary leading-relaxed mb-4">{user.bio}</p>}
      ...
      <main className="flex-1 min-w-0">
        <div className="flex border-b border-line mb-4">
          <button className="flex items-center gap-1.5 px-4 py-2.5 text-sm border-b-2 border-accent-orange text-primary">
            <GitBranch size={14} />
            Repositories
            <span className="font-mono text-xs bg-elevated border border-line px-1.5 py-0.5">{repos.length}</span>
          </button>
        </div>
        <div className="mb-4">
          <input
        ...
        {reposLoading ? (
          <div className="flex items-center justify-center py-12"><Spinner size="md" className="text-secondary" /></div>
        ) : filtered.length === 0 ? (
          <EmptyState
            icon={<GitBranch size={28} />}
            title={repos.length === 0 ? `${user.username} has no public repositories yet.` : 'No repositories match your filter.'}
          />
        ) : (
